...
export type WordReviewOutcome = 'known' | 'unknown' | 'skip';
...
export interface WordReviewRecordedAction {
  type: 'word_review_recorded';
  word_id: string;
  word_entry_id: string;
  word: string;
  outcome: WordReviewOutcome;
  status: WordReviewStatus;
}

export interface WordAddedToMistakesAction {
...
export type WordAiAction =
  | WordReviewRecordedAction
  | WordAddedToMistakesAction
  | WordDeckCreatedAction
  | WordAddedToDeckAction;
...
const DEFAULT_ENGLISH_SUBJECT = 'English';
...
function nextProgressAfter(
  ...
  const status = normalizeReviewStatus(current?.status || 'new');
  const base = {
    interval_days: Number(current?.interval_days || 0),
    correct_streak: Number(current?.correct_streak || 0),
    lapses: Number(current?.lapses || 0),
    reviewed_count: Number(current?.reviewed_count || 0),
    known_count: Number(current?.known_count || 0),
    unknown_count: Number(current?.unknown_count || 0),
  };
  ...
  if (outcome === 'unknown') {
    return {
      status: 'learning' as WordReviewStatus,
      due_at: reviewedAt,
      last_reviewed_at: reviewedAt,
      interval_days: 0,
      correct_streak: 0,
      lapses: base.lapses + 1,
      reviewed_count: base.reviewed_count + 1,
      known_count: base.known_count,
      unknown_count: base.unknown_count + 1,
      changed: true,
    };
  }

  const correctStreak = base.correct_streak + 1;
  let intervalDays = base.interval_days;
  let nextStatus: WordReviewStatus = status;
  ...
  if (status === 'new') {
    ...
    nextStatus = status === 'mastered' ? 'mastered' : 'review';
    intervalDays = Math.min(Math.max(base.interval_days * 2, 3), 180);
  }

  if (correctStreak >= 5 && intervalDays >= 30) {
    nextStatus = 'mastered';
  }
  ...
  return {
    ...
    lapses: base.lapses,
    reviewed_count: base.reviewed_count + 1,
    known_count: base.known_count + 1,
...
export async function recordWordReview(
  ...
  status: WordReviewStatus;
  due_at: string | null;
  actions: WordAiAction[];
  action: WordReviewRecordedAction;
  extra_actions: WordAiAction[];
}> {
  if (!VALID_REVIEW_OUTCOMES.includes(input.outcome)) {
    throw new WordToolError('invalid_request', 'Invalid review outcome', 400);
  }
  const mode = input.mode || 'sequential';
  if (!VALID_REVIEW_MODES.includes(mode)) {
    throw new WordToolError('invalid_request', 'Invalid review mode', 400);
  }
  const wordEntryId = input.word_entry_id || input.word_id;
  if (!wordEntryId) {
  ...
  const { error: eventError } = await ctx.supabase
    ...
    .insert({
      ...
      mode,
      source: ctx.source || (ctx.deviceId ? 'device' : 'web'),
      device_id: ctx.deviceId || null,
      conversation_id: ctx.conversationId || null,
      metadata: asJsonObject(input.metadata),
    });
  if (eventError) databaseError('recordWordReview.event', eventError);

  let persisted = existingProgress;
  if (nextProgress.changed) {
    const { data, error: progressError } = await ctx.supabase
      .from('word_progress')
      .upsert(
        {
          user_id: ctx.userId,
          ...
          due_at: nextProgress.due_at,
          last_reviewed_at: nextProgress.last_reviewed_at,
          interval_days: nextProgress.interval_days,
          correct_streak: nextProgress.correct_streak,
          lapses: nextProgress.lapses,
          reviewed_count: nextProgress.reviewed_count,
          known_count: nextProgress.known_count,
          unknown_count: nextProgress.unknown_count,
        },
        { onConflict: 'user_id,word_entry_id' }
      )
      .select('*')
      .single();
  ...
  const action: WordReviewRecordedAction = {
    type: 'word_review_recorded',
    word_id: wordEntryId,
    word_entry_id: wordEntryId,
    word: entry.word,
    outcome: input.outcome,
    status: normalizeReviewStatus(persisted?.status || nextProgress.status),
  };
  ...
  return {
    ...
    due_at: persisted?.due_at || nextProgress.due_at || null,
...
async function ensureWrongWordLink(
  ctx: WordToolContext,
  wordEntryId: string,
  word: string,
  entry: any
): Promise<WordAddedToMistakesAction> {
  const { data: existing, error: existingError } = await ctx.supabase
    .from('word_mistake_links')
    .select('problem_set_id, problem_id')
    .eq('user_id', ctx.userId)
    .eq('word_entry_id', wordEntryId)
    .maybeSingle();
  ...
  if (existing) {
    ...
    if (updateProblemError) {
      databaseError('ensureWrongWordLink.updateProblem', updateProblemError);
    }
    ...
    return {
      ...
      word_entry_id: wordEntryId,
      word,
      problem_set_id: existing.problem_set_id,
      problem_id: existing.problem_id,
      title: word,
    };
  }

  const subjectId = await ensureEnglishSubject(ctx.supabase, ctx.userId);
  const problemSetId = await ensureWrongWordProblemSet(
    ctx.supabase,
    ctx.userId,
    subjectId
  );
  ...
  const { data: problem, error: problemError } = await ctx.supabase
    ...
    .insert({
      ...
      title: word,
      content: buildWrongWordContent(entry),
      problem_type: 'short',
      status: 'wrong',
      correct_answer: word,
      solution_text: entry.meaning || '',
      assets: [],
      solution_assets: [],
      auto_mark: false,
    ...
    .select('id')
    .single();
  if (problemError) databaseError('ensureWrongWordLink.problem', problemError);
  ...
  const { error: joinError } = await ctx.supabase
    ...
    .upsert(
      {
        user_id: ctx.userId,
        problem_set_id: problem.id ? problemSetId : problemSetId,
        problem_id: problem.id,
      },
      { onConflict: 'problem_set_id,problem_id' }
    );
  if (joinError) {
    await deleteProblemBestEffort(ctx.supabase, ctx.userId, problem.id);
    databaseError('ensureWrongWordLink.problemSetJoin', joinError);
  }
    ...
    if (isUniqueConflict(error)) {
      ...
      if (!retryError && retry) {
        return {
          ...
          word_entry_id: wordEntryId,
          word,
          problem_set_id: retry.problem_set_id,
          problem_id: retry.problem_id,
          title: word,
        };
      }
    }
    databaseError('ensureWrongWordLink.link', error);
  }
...
async function ensureEnglishSubject(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      user_id: userId,
      name: DEFAULT_ENGLISH_SUBJECT,
      color: 'blue',
      icon: 'BookOpen',
    })
    .select('id')
    .single();
  if (error) databaseError('ensureEnglishSubject.create', error);
  return data.id;
}

async function ensureWrongWordProblemSet(
  supabase: SupabaseClient<any>,
  userId: string,
  subjectId: string
): Promise<string> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      user_id: userId,
      subject_id: subjectId,
      name: WORD_MISTAKE_SET_TITLE,
      description: 'Preset set for words marked unknown during Word review.',
  ...
  if (isUniqueConflict(error)) {
    const { data: retry, error: retryError } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (!retryError && retry) return retry.id;
  }

  databaseError('ensureWrongWordProblemSet.create', error);
}

function buildWrongWordContent(entry: any): string {
  return [
    `Word: ${entry.word}`,
    entry.phonetic ? `Phonetic: ${entry.phonetic}` : null,
    entry.meaning ? `Meaning: ${entry.meaning}` : null,
    entry.example ? `Example: ${entry.example}` : null,
    entry.example_translation
      ? `Example translation: ${entry.example_translation}`
      : null,
  ]
    .filter(Boolean)
    .join('\n');
...
async function deleteProblemBestEffort(
  supabase: SupabaseClient<any>,
  userId: string,
  problemId: string
) {
  const { error } = await supabase
    ...
    .eq('user_id', userId);
  if (error) {
    logger.warn('Failed to delete unused wrong-word problem', {
      component: 'Words',
      action: 'deleteProblemBestEffort',
      userId,
      problemId,
    });
  }
