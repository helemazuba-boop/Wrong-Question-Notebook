...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  "$path = '%ENV_FILE%';" ^
  "$required = @('NEXT_SERVER_ACTIONS_ENCRYPTION_KEY','NEXT_PUBLIC_SUPABASE_URL','NEXT_PUBLIC_SUPABASE_PUBLISHABLE_OR_ANON_KEY','WQN_SUPABASE_EXPECTED_HOST','SUPABASE_SECRET_KEY','DASHSCOPE_API_KEY','WQN_ESP32_AI_PUBLIC_BASE_URL','WQN_ESP32_AI_AUDIO_URL_SECRET','STEP_API_KEY','WQN_REALTIME_PROXY_SECRET','WQN_INTERNAL_API_BASE','WQN_INTERNAL_API_ALLOWED_HOST');" ^
  "$placeholders = @('sb_publishable_replace_me','sb_secret_replace_me','your_dashscope_api_key_here','replace_with_32_byte_base64_key','replace_with_long_random_secret','https://data.example.invalid','https://your-domain.com','replace_with_stepfun_api_key');" ^
  "$vars = @{};" ^
